...
def batchnorm_backward(dout, cache):
    ...
    # might prove to be helpful.                                              #
    ###########################################################################
    x, mu, var, std, x_nor, gamma, beta = cache['x'], cache['mu'], cache['var'], cache['std'], cache['x_nor'], cache['gamma'], cache['beta']
    N, D = x.shape
    ...
    #[8]. e = gamma * x_nor
    dgamma = np.sum(de * x_nor, axis=0)
    dx_nor = de * gamma
    
    #[7]. x_nor = a1 * c
    da1 = dx_nor * (1/std)
    dc = np.sum(dx_nor * (x-mu), axis=0)
    ...
    #[4]. var = 1/N * sum_{i=1}^N b_i
    db = (1/N * np.ones(x.shape)) * dvar
    ...
    #[2]. a = x-mu
    dx2 = da1 + da2
    dmu = -np.sum(da1 + da2, axis=0)
    
    #[1]. mu = 1/N sum_{i=1}^N x_i
    dx1 = (1/N * np.ones(x.shape)) * dmu
