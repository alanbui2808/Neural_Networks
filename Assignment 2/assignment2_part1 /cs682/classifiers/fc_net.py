...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # First layer + ReLU
        FC1, FC1_cache = affine_relu_forward(X, W1, b1)
        
        # Second layer
        scores, FC2_cache = affine_forward(FC1, W2, b2)
        ...
        # Gradient
        dFC1, dW2, db2 = affine_backward(dL_dsoftmax, FC2_cache)
        dX, dW1, db1 = affine_relu_backward(dFC1, FC1_cache)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            layer_id = i + 1
            X = W = b = gamma = beta = bn_params = None
            ...
            if layer_id == self.num_layers: # last layer is not Affine-Relu
                compute_layer = affine_forward
                layer_params = (X, W, b)
            else:        
                compute_layer = affine_normalize_relu_forward
                layer_params = (X, W, b, gamma, beta, bn_params, self.normalization)
        ...
        for i in reversed(range(self.num_layers)):
            ...
            if layer_id == self.num_layers: # last layer is not Affine-ReLU
                ...
                compute_gradient = affine_normalize_relu_backward
                gradient_params = (dout, cache, self.normalization)
